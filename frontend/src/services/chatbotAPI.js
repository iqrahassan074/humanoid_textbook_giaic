...
const chatbotAPI = {
  ...
  askQuestion: async (question, chapterId = null) => {
    try {
      const response = await apiClient.post('/api/v1/chatbot/ask', {
        question,
  ...
  getQuestionHistory: async () => {
    try {
      const response = await apiClient.get('/api/v1/chatbot/history');
      return response.data;
    } catch (error) {
  ...
  testRagPipeline: async () => {
    try {
      const response = await apiClient.post('/api/v1/chatbot/test-rag');
      return response.data;
    } catch (error) {
